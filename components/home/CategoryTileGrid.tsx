      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4">
        {groups.map((g) => (
          <button
            ...
            className="group relative aspect-[4/5] overflow-hidden rounded-3xl text-left shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl motion-reduce:transition-none motion-reduce:hover:translate-y-0"
          >
            {g.image ? (
            ...
            <div className="absolute inset-x-0 bottom-0 p-4">
              <h3 className="text-base font-extrabold leading-tight tracking-tight text-white drop-shadow-md sm:text-lg">
                {t(g.titleKey, lang)}
              </h3>
            ...
            <span
              aria-hidden
              className="absolute bottom-4 right-4 grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white/10 text-white ring-1 ring-white/25 backdrop-blur-md transition-colors duration-300 group-hover:bg-amber-300 group-hover:text-slate-900 motion-reduce:transition-none"
            >
